...
def plot_input_mob(ParFileDic,ax=None,x_unit='nm',y_unit='m',background=True,**kwargs):
    ...
    background : bool, optional
        if True, puts a background color for the different layers, by default True
    **kwargs : dict, optional
        kwargs control, can be ['xscale','yscale','xaxis_label','yaxis_label','xunit_fact','yunit_fact'], by default {'xscale':'linear','yscale':'log'}  
        including:  
            xscale : str, optional
                scale of the x-axis, can be ['linear','log'], by default 'linear'
            yscale : str, optional
                scale of the y-axis, can be ['linear','log'], by default 'log'
            xaxis_label : str, optional
                label of the x-axis, by default None
            yaxis_label : str, optional
                label of the y-axis, by default None
            xunit_fact : float, optional
                factor to convert the x-axis, by default 1
            yunit_fact : float, optional
                factor to convert the y-axis, by default 1
    ...
    xaxis_label = kwargs.get('xaxis_label', None)
    yaxis_label = kwargs.get('yaxis_label', None)
    xunit_fact = kwargs.get('x_unit_fact', 1)
    yunit_fact = kwargs.get('y_unit_fact', 1)
...
def plot_input_dens(ParFileDic,ax=None,dens=['Bulk_tr'],x_unit='nm',y_unit='m',y2_unit='m',**kwargs):
    """Plot the input mobility from the device_parameters.txt
    ...
    **kwargs : dict, optional
        kwargs control, can be ['xscale','yscale','xaxis_label','yaxis_label','y2axis_label','xunit_fact','yunit_fact','y2unit_fact'], by default {'xscale':'linear','yscale':'log'}  
        including:  
            xscale : str, optional
                scale of the x-axis, can be ['linear','log'], by default 'linear'
            yscale : str, optional
            ...
            yaxis_label : str, optional
                label of the y-axis, by default None
            y2axis_label : str, optional
                label of the y2-axis, by default None
            xunit_fact : float, optional
                factor to convert the x-axis, by default 1
            yunit_fact : float, optional
                factor to convert the y-axis, by default 1
            y2unit_fact : float, optional
                factor to convert the y2-axis, by default 1
    ...
    xaxis_label = kwargs.get('xaxis_label', None)
    yaxis_label = kwargs.get('yaxis_label', None)
    y2axis_label = kwargs.get('y2axis_label', None)
    xunit_fact = kwargs.get('xunit_fact', 1)
    yunit_fact = kwargs.get('yunit_fact', 1)
    y2unit_fact = kwargs.get('y2unit_fact', 1)
    ...
    # Convert in x-axis
    L = L * xunit_fact
    L_LTL = L_LTL * xunit_fact
    L_RTL = L_RTL * xunit_fact
    Lac = L - L_LTL - L_RTL
    # Convert in y-axis
    if yaxis_label is None:
        if y_unit == 'cm':
    ...
    if plot_y2:
        if y2axis_label is None:
            if y2_unit == 'cm':
                ...
                print('y2_unit is wrong so [m] is used. ')
...
def plot_input_nrj_diag(ParFileDic,ax,x_unit='nm',**kwargs):
...
def plot_input_SRH_lifetime(ParFileDic,ax,dens=['Bulk_tr','St_L','St_R'],x_unit='nm',y_unit='s',y2_unit='m',**kwargs):
    """Plot the input mobility from the device_parameters.txt
    ...
    ax : axes
        axes object where the plot will be done
    dens : list, optional
        list of the densities type to plot, can be ['Bulk_tr','St_L','St_R'], by default ['Bulk_tr','St_L','St_R']
    x_unit : str, optional
        unit to plot the x-axis, can be ['nm','um','m'], by default 'nm'
    y_unit : str, optional
    ...
    **kwargs : dict, optional
    ...
    if 'St_L' in dens or 'St_R' in dens:
        if St_L > 0 or St_R > 0:
            plot_y2 = True
            ax2 = ax.twinx() # instantiate a second axes that shares the same x-axis
    line_thick = 2
    ...
    if Bulk_tr > 0 or St_L > 0 or St_R > 0:
        ...
        if L_LTL > 0:
            ...
            if TLsTrap == 1 and 'Bulk_tr' in dens and Bulk_tr > 0:
                ...
                ax.plot([0,L_LTL],[tau_p,tau_p], color = 'k', linewidth = line_thick, linestyle = 'dashed')
            
            if St_L > 0 and 'St_L' in dens:
